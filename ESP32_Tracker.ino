  ...
  for(int i=Day; i<=day(); i++)
  {
    if(GPX_FileInfo(Year, Month, i, &Meters, &Segments, &Tracks, &Stops, &Riding, &Elapsed, StartTime, EndTime, &AvgSpeed, &MaxSpeed, &StopStr))
    {
      ...
      if(Meters >= DataRec.MinDailyDistance)
      {
        ...
        if(MaxSpeed > Config.TripMaxSpeed) Config.TripMaxSpeed = MaxSpeed;
        Config.SeasonDistance += (Meters * 1000.0); 
        Config.SeasonRiding += Riding;
        if(MaxSpeed > Config.SeasonMaxSpeed) Config.SeasonMaxSpeed = MaxSpeed;
      }
      else
      {
...
void GPX_TrackPoint(void)
{
  ...
  file.printf("</distance><speed>%s</speed><time>%s</time></trkpt>\n", String(Speed, 1).c_str(), getTime(false));
  file.close();
...
bool Read_Config_File(void)
{
  ...
  file.close();
  if(strlen(DataRec.SSID2) == 0) Okay = false;
  if(strlen(DataRec.PASS2) == 0) Okay = false;
  ...
  if(strlen(DataRec.SSID1) == 0) strcpy(DataRec.SSID1, DataRec.SSID2);
  if(strlen(DataRec.PASS1) == 0) strcpy(DataRec.PASS1, DataRec.PASS2);
...
// TODO: Open monthly weather file as well  
  ...
  for(i=1, j=1; i<32; i++, j++)
  {
    ...
    if(!Media)
    {
      // No media files, what about a text file?
...
void  DailyRide(WiFiClient WebClient, int Year, int Month, int Day, int PageType)
{
  ...
  WebClient.printf("%s, %d Stops, %d Tracks\r\n", Line.c_str(), Stops, Tracks);
  if(Riding != 0) 
  {
