...
// Modifications are marked with a "[CGB]" comment

#include "TinyGPSMod.h" // [CGB]
  ...
  if (isChecksumTerm)
  {
    ...
    if (checksum == parity)
    {
      ...
      case GPS_SENTENCE_GPRMC:
        ...
        {
 //          location.commit(); // [CGB]
    		   location.lastCommitTime = millis(); // [CGB]
           speed.commit();
           course.commit();
...
//          location.commit();  // [CGB]
    ...
    switch(COMBINE(curSentenceType, curTermNumber))
  {
    ...
    case COMBINE(GPS_SENTENCE_GPRMC, 7): // Speed (GPRMC)
      speed.set(term);
      break;
    case COMBINE(GPS_SENTENCE_GPRMC, 8): // Course over ground-true (GPRMC)
      course.set(term);
      break;
...
//      sentenceHasFix = term[0] > '0';
      sentenceHasFix = (term[0] > '0' && term[0] != '6');  // [CGB]
      break;
...
{
   updated = false;
//   double ret = rawLatData.deg + rawLatData.billionths / 1000000000.0; // [CGB] see: https://github.com/mikalhart/TinyGPSPlus/pull/47
...
{
   updated = false;
//   double ret = rawLngData.deg + rawLngData.billionths / 1000000000.0; // [CGB] see: https://github.com/mikalhart/TinyGPSPlus/pull/47
